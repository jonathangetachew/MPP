...
public class Main {
	...
	public static List<String> allWhoseZipContains3() {
		...
		return mems.stream().filter(libraryMember -> libraryMember.getMemberId().contains("3"))
	...
	//Returns a list of all isbns of books having at least two copies
		public static List<String> allHavingAtLeastTwoCopies() {
			...
			return bs.stream().filter(book -> book.getCopies().length >= 2)
					.map(Book::getIsbn)
					.collect(Collectors.toList());
	...
	public static List<String> allHavingMultipleAuthors() {
		...
		return bs.stream().filter(book -> book.getAuthors().stream().count() >= 2)
				.map(Book::getIsbn)
				.collect(Collectors.toList());
